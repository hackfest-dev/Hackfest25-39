...
const SectorDashboard = () => {
  ...
  const sectorConfig = {
    ...
    'OB Removal': {
      ...
      breakdown: {
        diesel: { label: 'Equipment  Emissions' },
        explosives: { label: 'Blasting  Emissions' },
  ...
  // In SectorDashboard.jsx, modify the Support Infrastructure config:
'Support Infrastructure': {
  inputs: {
    dieselGenerators: { label: 'Diesel Generators', unit: 'L' },
    workshopElectricity: { label: 'Workshop Electricity', unit: 'kWh' }
  },
  breakdown: {
    diesel: { label: 'Generator Emissions' },
    electricity: { label: 'Electricity Emissions' }
  }
},
  'Coal Processing': {
    inputs: {
      coalWashed: { label: 'Coal Processed', unit: 'tonnes' },
      electricity: { label: 'Electricity Used', unit: 'kWh' },
      waterUsed: { label: 'Water Consumed', unit: 'm³' }
    },
    breakdown: {
      electricity: { label: 'Processing Emissions' },
      water: { label: 'Water Treatment Emissions' }
    }
  },

  'Exploration': {
    inputs: {
      drillHours: { label: 'Drilling Hours', unit: 'hrs' },
      dieselUsed: { label: 'Diesel Used', unit: 'L' }
    },
    breakdown: {
      drilling: { label: 'Drilling Emissions' },
      diesel: { label: 'Diesel Emissions' }
    }
  }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const sectorRes = await fetch(`${apiBaseURL}/api/sector/check-session`, { credentials: 'include' });
        ...
        setCurrentSector({
          ...
          type: sectorData.sector_name?.trim()
        });

        const emissionsRes = await fetch(`${apiBaseURL}/api/emissions/${sectorData.sector_id}`, { 
          credentials: 'include' 
        });
  ...
  const verifiedEmissions = emissionsData.filter(entry => entry.verified);
  ...
  const chartData = {
    ...
    datasets: [{
      ...
      data: Array(12).fill(0).map((_, i) => {
        const entry = verifiedEmissions.find(e => 
          ...
          <Line 
            ...
            options={{
              ...
              plugins: {
                title: {
                  display: true,
                  text: `Verified Emissions Overview - ${selectedYear}`
                }
              }
            }}
          />
              ...
              <div key={`${entry.year}-${entry.month}`} className="emission-card">
                <div className="card-header">
                  ...
                  <div className="status-badges">
                    <span className={`verification-badge ${entry.verified ? 'verified' : 'unverified'}`}>
